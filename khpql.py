...
# /usr/bin/python3 does not work; it appearst to be an older version. Alternatively python3 points to the one in localm so that works, too...
    ...
    for the_line in subject:
        ...
        for the_target_line in object:
            ...
            if skip_or_check(the_target_line):
                continue
            result = fuzz.partial_ratio_alignment(the_line, the_target_line, processor=utils.default_process, score_cutoff=SCORE)
            if result.score > 0:
                print(i, "\t",
                 the_line.rstrip(), "\t",
                 j, "\t",
